createVICVegRVicNLDAS11 <- function(vicBdry = 0, vicGrid = 0, vicCellNmbr = 0,
                             resolutionKm = NULL){
  # k. westrick 2/5/2019
  ...
  rcl <- cbind(glcRemapTable$glcVegCode,glcRemapTable$remap11)
  ...
  processVeg <- function(cellNmbr){
    ...
    remapIndices <- glcRemapTable$remap11[match(veg$veg_type,glcRemapTable$glcVegCode)]
    # root zones
    ...
    # LAI
    laiMax <- matrix(combinedVegTable$LAImax[glcRemapTable$remap11][remapIndices],nc=1)
    laiMaxVector <- matrix(rep(laiMax,12),nc=12) 
    laiMin <- matrix(combinedVegTable$LAImin[glcRemapTable$remap11][remapIndices],nc=1)
  ...
  processVeglib <- function(){
    Class <- glcRemapTable$glcMapsTo11
    remapIndices <- glcRemapTable$remap11[match(Class,glcRemapTable$glcVegCode)]
    OvrStry <- combinedVegTable$overstory[remapIndices]#rep(0,20)
    Rarc <- combinedVegTable$rarc[remapIndices]
    Rmin <- combinedVegTable$rmin[remapIndices]
    WIND_H <- combinedVegTable$wind_h[remapIndices]
    RGL <- combinedVegTable$RGL[remapIndices]
    SolAtn <- combinedVegTable$rad_atten[remapIndices]
    WndAtn <- combinedVegTable$wind_atten[remapIndices]
    Trunk <- combinedVegTable$trunk_ratio[remapIndices]
    MapsTo <- combinedVegTable$vegDescription[remapIndices]
    Original <- glcRemapTable$glcVegDesc
    # LAI
    ...
    # Albedo
    ALBmat <- matrix(combinedVegTable$albedo[glcRemapTable$remap][remapIndices],nc=1)
    ALB <- matrix(rep(ALBmat,12),nc=12) 
    colnames(ALB) <- paste0(toupper(month.abb),".ALB")
    # Roughness
    ROUmat <- matrix(combinedVegTable$roughnessLengthM[glcRemapTable$remap][remapIndices],nc=1)
    ROU <- matrix(rep(ROUmat,12),nc=12) 
    colnames(ROU) <- paste0(toupper(month.abb),".ROU")
    # Displacement
    DISmat <- matrix(combinedVegTable$displacement[glcRemapTable$remap][remapIndices],nc=1)
