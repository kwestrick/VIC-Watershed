createVICParameters <- function(vicBdry = 0, vicGrid = 0, thisFilename = "Test",
  ...
  var_expt <- ncvar_def("expt","N/A",list(dimLayers, dimLon,dimLat),missingValue,prec = "double", longname = "expt")
  var_Ksat <- ncvar_def("Ksat","mm/day",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "kSat")
  var_phi_s <- ncvar_def("phi_s","mm/mm",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "phi_s")
  var_init_moist <- ncvar_def("init_moist","mm",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "init_moist")
  var_elev <- ncvar_def("elev","",list(dimLon,dimLat),missingValue,prec = "double", longname = "elev")
  var_depth <- ncvar_def("depth","m",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "depth")
  ...
  var_bubble <- ncvar_def("bubble","cm",list(dimLayers, dimLon,dimLat),missingValue,prec = "double", longname = "bubble")
  var_quartz <- ncvar_def("quartz","fraction",list(dimLayers, dimLon,dimLat),missingValue,prec = "double", longname = "quartz")
  var_bulk_density <- ncvar_def("bulk_density","kg/m3",list(dimLayers, dimLon,dimLat),missingValue,prec = "double", longname = "bulk_density")
  var_soil_density <- ncvar_def("soil_density","kg/m3",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "soil_density")
  var_off_gmt <- ncvar_def("off_gmt","hours",list(dimLon,dimLat),missingValue,prec = "double", longname = "off_gmt")
  var_Wcr_FRACT <- ncvar_def("Wcr_FRACT","fraction",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "Wcr_FRACT")
  var_Wpwp_FRACT <- ncvar_def("Wpwp_FRACT","fraction",list(dimLayers,dimLon,dimLat),missingValue,prec = "double", longname = "Wpwp_FRACT")
  ...
  var_resid_moist <- ncvar_def("resid_moist","",list(dimLayers, dimLon,dimLat),missingValue,prec = "double", longname = "resid_moist")
  ...
  if(resolutionKm > thresholdResolutionMakeSnowBands) {
    ...
    var_areaFract <- ncvar_def("areaFract","fraction",list(dimSnowBand,dimLon,dimLat),missingValue,prec = "double")
    var_elevation <- ncvar_def("elevation","m",list(dimSnowBand,dimLon,dimLat),missingValue,prec = "double", longname = "")
    var_Pfactor <- ncvar_def("Pfactor","fraction",list(dimSnowBand,dimLon,dimLat),missingValue,prec = "double", longname = "")
  ...
  var_Nveg <- ncvar_def("Nveg","N/A",list(dimLon,dimLat),
                        prec = "integer", longname = "Nveg")
  var_Cv <- ncvar_def("Cv","fraction",list(dimVegClasses,dimLon,dimLat),missingValue,
                      prec = "double", longname = "Fraction of grid cell covered by vegetation tile")
  var_root_depth <- ncvar_def("root_depth","m",list(dimVegClasses,dimRootZones,dimLon,dimLat),missingValue,
                              prec = "double", longname = "Root zone thickness (sum of depths is total depth of root penetration)")
  var_root_fract <- ncvar_def("root_fract","fraction",list(dimVegClasses,dimRootZones,dimLon,dimLat),missingValue,
                              prec = "double", longname = "Fraction of root in the current root zone")
  var_LAI <- ncvar_def("LAI","m2/m2",list(dimVegClasses,dimMonth,dimLon,dimLat),missingValue,
  ...
  var_rmin <- ncvar_def("rmin","s/m",list(dimLon,dimLatdim,VegClasses),missingValue,
  ...
  var_veg_rough <- ncvar_def("veg_rough","m",list(dimVegClasses,dimMonth,dimLon,dimLat),missingValue,
                             prec = "double", longname = "Vegetation roughness length (typically 0.123 * vegetation height)")
  var_displacement <- ncvar_def("displacement","m",list(dimVegClasses,dimMonth,dimLon,dimLat),missingValue,
  ...
  outArray <- as.array(rotate(outBrick))
