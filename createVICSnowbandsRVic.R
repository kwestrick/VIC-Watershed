createVICSnowbandsRVic <- function(vicBdry = 0, vicGrid = 0, vicCellNmbr = 0,
                                resolutionKm = NULL,  
                                thresholdResolutionMakeSnowBands = 3){
  # k. westrick 2/5/2019
  ...
  if(resolutionKm > thresholdResolutionMakeSnowBands) {
    ...
    retrieveSnowBand <- function(cellNmbr){
      ...
      if(!all(is.na(getValues(thisElev)))){ #check if any of this grid falls in watershed
        # plot(thisElev)
        # plot(grdPolys, border="blue",add = T)
        # plot(vicBdry, add=T, border="grey40")
        ...
        outFrame <- data.frame(matrix(c(cellNmbr,areaFrac,elevBands,pFactor),nr=1))
        colnames(outFrame) <- c("GRID",
                                paste0("AFRAC_",1:maxSnowBands),
                                paste0("ELEV_",1:maxSnowBands),
                                paste0("PFRAC_",1:maxSnowBands))
        
        return(outFrame)
      } else {
        outFrame <- data.frame(matrix(c(cellNmbr,rep(0,maxSnowBands),
                                        rep(0,maxSnowBands),PFRAC_=rep(0,maxSnowBands)),nr=1))
        colnames(outFrame) <- c("GRID",
                                paste0("AFRAC_",1:maxSnowBands),
                                paste0("ELEV_",1:maxSnowBands),
                                paste0("PFRAC_",1:maxSnowBands))
    ...
    snowband <- do.call(rbind, lapply(1:nmbrCells, retrieveSnowBand))
    return(snowband)
  } 
}
