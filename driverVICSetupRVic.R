driverVICSetup <- function(watershedPourPointName = "STEHEKIN"){
  ...
  soil <- createVICSoilRVic(vicBdry = thisWatershedBdry,vicGrid = vicGrid, thisFilename = thisFilename,
  ...
  ################
  yr <- "1980"
  forcing <- createVICForcingRVic(maskRaster = vicGrid, yr = yr,
                   thisFilename = thisFilename)
  ...
  saveFile <- paste0(climaticsOpDir,"Run/InputRLists/",
                     watershedPourPointName,"_",resolutionKm,"km_",yr,".rds")
